...
const Students = ({ data, setUsername, students, pagination }) => {
  ...
  const handleSubmitEdit = async (e) => {
    e.preventDefault();
    await axios
      .post(`/api/admin/students/${selectedRow.nisn}`, selectedRow, {
  ...
  const searchData = async () => {
    ...
    await axios
      .get(`/api/admin/students/${keyword}`, {
        withCredentials: true,
      })
      .then(async (response) => {
  ...
  const deleteNisn = async () => {
    try {
      await axios
        .delete(`/api/admin/students/${selected}`, {
          withCredentials: true,
        })
        .then(async (response) => {
    ...
    <>
      ...
      <Box bg={"#614BFE"} pb={10}>
        ...
        <Box bg={"white"} mx={10} p={10} borderRadius={6}>
                  ...
                  <Tr key={index}>
                    ...
                    <Td>
                      <Stack direction={"row"} spacing={5} justifyContent={"center"}>
                        <IconButton variant="solid" colorScheme="red" onClick={() => deleteHandler(item.nisn)} aria-label="Delete Siswa" icon={<DeleteIcon />} />
                        <IconButton variant="solid" colorScheme="blue" onClick={() => handleEdit(item)} aria-label="Delete Siswa" icon={<EditIcon />} />
                      </Stack>
                    </Td>
                  </Tr>
                ))}
          ...
          <Modal isOpen={isEditOpen} isCentered onClose={() => setIsEditOpen(false)}>
            <ModalOverlay />
            <ModalContent>
          ...
          <AlertDialog motionPreset="slideInBottom" leastDestructiveRef={cancelAlertRef} onClose={onClose} isOpen={isOpen} isCentered>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogCloseButton />
              <AlertDialogBody>Apakah kamu yakin ingin menghapus data?</AlertDialogBody>
              <AlertDialogFooter>
          ...
          <Box mt={5}>
            <Button onClick={() => paginate(value - 1)}>-</Button>
            <Box as="span" w="200px" mx="24px">
              {value}
            </Box>
            <Button disabled={value + 1 === parseInt(pagination.max)} onClick={() => paginate(value + 1)}>
              +
            </Button>
          </Box>
        </Box>
